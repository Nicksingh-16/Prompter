...
// Question/informational starters — catches "whats", "what is", "how do" etc.
// including missing-apostrophe variants like "whats" for "what's".
const QUESTION_STARTERS: &[&str] = &[
    "what ", "whats ", "what's ", "what is ", "what are ", "what was ",
...
pub struct SignalVector {
    ...
    pub greeting_position: Option<usize>,
    ...
    // Question/informational signal — THE fix for "whats the cutoff of X" → Social
    pub is_question_starter: bool,
}
...
pub fn position_weight(offset: usize, total_len: usize) -> f32 {
    ...
    if pct < 0.05      { 4.0 }
    else if pct < 0.15 { 2.5 }
    else if pct < 0.30 { 1.5 }
    else if pct > 0.85 { 2.0 }
    else               { 1.0 }
}
...
pub fn detect_salutation_structure(text: &str) -> bool {
    ...
    let has_closer = CLOSER_WORDS.iter().any(|c| last.starts_with(c) || last.ends_with(c));
...
pub fn extract_signals(text: &str, features: &FeaturesOutput, _lang: &LanguageContext) -> SignalVector {
    ...
    let greeting_position = GREETING_WORDS.iter().filter_map(|g| lower.find(g)).min();
    ...
    let subject_line      = lower.starts_with("subject:") || lower.starts_with("re:") || lower.starts_with("fwd:");
    let has_salutation_structure = detect_salutation_structure(text);

    // Question/informational signal — critical for preventing "whats X" → Social
    let is_question_starter = QUESTION_STARTERS.iter().any(|q| lower.starts_with(q));
...
fn score_email(sv: &SignalVector) -> f32 {
    ...
    if sv.greeting_found {
        let pw = sv.greeting_position.map(|p| if p < 5 { 2.0 } else { 0.5 }).unwrap_or(0.5);
        score += 1.5 * pw;
    }
...
fn score_chat(sv: &SignalVector) -> f32 {
    ...
    if sv.sentence_count <= 3      { score += 0.3; } // reduced — short text alone ≠ chat
    if sv.has_salutation_structure { score -= 2.0; }
    if sv.contraction_rate > 0.3   { score += 0.8; }
    if sv.word_count < 30          { score += 0.2; } // reduced — short ≠ chat
    // Questions are informational, not chat
    if sv.is_question_starter      { score -= 1.5; }
    score.max(0.0)
}
...
fn score_prompt(sv: &SignalVector) -> f32 {
    ...
    // Question starters (what/how/why) are informational/prompt-like
    if sv.is_question_starter      { score += 1.5; }
    if sv.question_count > 0       { score += 1.0; } // having ? is also a signal
...
fn score_knowledge(sv: &SignalVector) -> f32 {
    ...
    if sv.is_question_starter      { score += 1.0; }
...
fn score_social(sv: &SignalVector) -> f32 {
    let mut score = 0.0_f32;
    let social_kw = sv.keyword_scores.get(&Intent::Social).copied().unwrap_or(0.0);
    score += social_kw;
    // Short-text bonus ONLY when there is an actual social signal present.
    // Without this, any short generic message (questions, chat, notes)
    // would score high as Social just by being short.
    if sv.word_count < 60 && (social_kw > 0.0 || sv.exclamation_count > 0) { score += 0.8; }
    if sv.exclamation_count > 1    { score += 0.8; }
    if sv.has_salutation_structure { score -= 2.5; }
    if sv.formality <= 4           { score += 0.5; }
    // Questions are almost never social posts
    if sv.is_question_starter      { score -= 2.5; }
    if sv.question_count > 0       { score -= 1.0; }
    score.max(0.0)
}
...
fn make_candidate(intent: &Intent, confidence: f32) -> IntentCandidate {
    ...
    IntentCandidate { intent: intent.clone(), confidence, label, mode, reason }
}
...
fn candidate_meta(intent: &Intent) -> (&'static str, &'static str, &'static str) {
    match intent {
        ...
        Intent::Prompt    => ("AI Prompt",       "Prompt",       "question or instruction detected"),
        ...
        Intent::Social    => ("Social post",     "Casual",       "punchy, social-media signals"),
        Intent::General   => ("General",         "Correct",      "no strong pattern detected"),
    }
}

pub fn normalize_scores(raw: HashMap<Intent, f32>) -> IntentResult {
    let total: f32 = raw.values().sum();
    if total < 0.01 { return IntentResult::default_general(); }
...
pub fn classify_deep(text: &str, features: &FeaturesOutput, lang: &LanguageContext) -> IntentResult {
...
pub fn suggest_mode(intent_result: &IntentResult, features: &FeaturesOutput) -> String {
    match &intent_result.primary.intent {
        Intent::Email   => if features.formality >= 6 { "Professional".into() } else { "Casual".into() },
        Intent::Chat    => "Casual".into(),
        Intent::Prompt  => if features.imperative_count > 0 { "Prompt".into() } else { "Correct".into() },
        Intent::Knowledge => "Knowledge".into(),
        Intent::Report  => if features.word_count > 200 { "Summarize".into() } else { "Professional".into() },
        Intent::Social  => "Casual".into(),
        Intent::General => if features.word_count > 150 { "Summarize".into() } else { "Correct".into() },
    }
}