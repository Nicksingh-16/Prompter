...
const TonePill = ({ score, friction }: { score: number; friction: string[] }) => {
  const impact = (() => {
    if (friction.length > 0) return { color: '#f59e0b', label: 'Potential conflict', verdict: 'Reads as Tense/Impatient' };
    if (score >= 4) return { color: '#10b981', label: 'Very Warm', verdict: 'Reads as Enthusiastic' };
    if (score >= 1) return { color: '#3b82f6', label: 'Polished', verdict: 'Reads as Professional' };
    if (score <= -3) return { color: '#ef4444', label: 'Urgent', verdict: 'Reads as High Pressure' };
    return { color: 'var(--text-dim)', label: 'Neutral', verdict: 'Reads as Calm/Clear' };
  })();

  return (
    <div style={{
      display: 'flex', flexDirection: 'column', gap: '2px',
      fontSize: '10.5px', marginTop: '6px',
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
        <span style={{
          background: impact.color, width: '6px', height: '6px', borderRadius: '50%',
          boxShadow: `0 0 8px ${impact.color}`
        }} />
        <span style={{ fontWeight: 600, color: 'var(--text)' }}>{impact.verdict}</span>
      </div>
      {friction.length > 0 && (
        <span style={{ color: impact.color, opacity: 0.8, fontSize: '9px', fontStyle: 'italic' }}>
          Avoid: "{friction[0]}"
        </span>
      )}
    </div>
        ...
        {expanded && (
          ...
          >
            <div style={{
              ...
              {(['Correct', 'Professional', 'Casual', 'Summarize', 'Email', 'Translate', 'Strategist', 'Prompt', 'Knowledge', 'Custom'] as Mode[]).map(m => (
                <button
                  key={m}
...
function App() {
  ...
  const handleInsert = async () => {
    ...
    // Double-ensure it's on the clipboard before calling backend inject
    try {
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
          ...
          <span style={{ fontWeight: 600, fontSize: '13.5px' }}>AI Prompter</span>
          <button
